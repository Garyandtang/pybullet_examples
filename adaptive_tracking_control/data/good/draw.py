...
def main():
    ...
    # plot last one  init trajectory and trained trajectory and ref trajectory
    plt.figure()
    plt.grid(True)
    plt.xticks(fontsize=font_size - 2)
    plt.yticks(fontsize=font_size - 2)
    # last one
    plt.plot(x_init_container[-1, :], y_init_container[-1, :], linewidth=line_width)
    plt.plot(x_trained_container[-1, :], y_trained_container[-1, :], linewidth=line_width)
    # scatter ref trajectory with star
    # plt.scatter(ref_state[0, :], ref_state[1, :], marker='*', s=100)


    # plt.plot(ref_state[0, :], ref_state[1, :], linewidth=line_width)
    plt.xlabel("$x~(m)$", fontsize=font_size)
    plt.ylabel("$y~(m)$", fontsize=font_size)
    name = "last_trajectory.jpg"
    plt.show()
